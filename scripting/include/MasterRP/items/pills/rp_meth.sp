...
public Action:CheckPillsItemsToPillsKitchen(Client, Y, Float:EntOrigin[3])
{
	...
	new Ammonia = CheckAmmoniaTankDistanceToPillsLab(Client, Y);

	new SAcidTub = CheckSAcidTubCanDistanceToPillsLab(Client, Y);

	new Toulene = CheckTouleneDistanceToPillsLab(Client, Y);

	//Check:
	if(Propane > 0 && Ammonia > 0 && Acetone > 0 && Acid > 0 && Sodium > 0)
	{
		...
		//Declare:
		new Id = GetAmmoniaTankIdFromEnt(Ammonia);
		...
		//Declare:
		Id = GetSAcidTubIdFromEnt(Phosphoru);

		new Float:UsedFuel = GetRandomFloat(0.01, 0.009);

		SetSAcidTubFuel(Client, Id, (GetSAcidTubFuel(Client, Id) - UsedFuel));

		//Declare:
		Id = GetTouleneIdFromEnt(Sodium);

		UsedGrams = GetRandomFloat(0.1, 0.05);

		SetTouleneGrams(Client, Id, (GetTouleneGrams(Client, Id) - UsedGrams));
...
public CheckAmmoniakDistanceToPillsLab(Client, Y)
{
